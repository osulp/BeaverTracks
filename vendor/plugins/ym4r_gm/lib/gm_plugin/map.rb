module Ym4r
  module GmPlugin 
    ...
    class GMap
      ...
      def directions(div,saddr,eaddr)
        @init << "directions = new GDirections(#{@variable}, document.getElementById('#{div}'));"
        @init << "var opts = {};  opts.travelMode = G_TRAVEL_MODE_WALKING;"
        # Error Handling
        @init << "GEvent.addListener(directions, 'error', function() {
...
"
        ...
        @init << "directions.load('from:#{saddr} to:#{eaddr}',opts);"
      end
